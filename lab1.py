...
def lcremovep(lst, pred):
    res=list()
    res=[ i  for i in lst if not pred(i)]
    return res
...
def doubletree(tree):
    ...
    for i in range(len(tree)):
          if type(tree[i])==list:
            tree[i]= doubletree(tree[i])
          elif type(tree[i])==int:
              tree[i]=2*tree[i]
          elif type(tree[i])==int:
...
def types(tree):
    ...
    for i in range(len(tree)):
        if type(tree[i])==list:
        ...
        elif type(tree[i])==bool:
            res+=['bool']

        elif type(tree[i])==dict:
            res+=['dict']

        elif type(tree[i])==float:
            res+=['float']
            ...
            res+=['str']
        elif type(tree[i])==tuple: res+=['tuple']
